...
function EditStudentForm({ onDelete }) {
    ...
    const fetchStudents = async (parentId) => {
        try {
            const response = await axios.get(`http://localhost:5000/api/student/parent/${parentId}`);
    ...
    const handleSaveStudent = async () => {
        try {
            await axios.put(`http://localhost:5000/api/student/update/${selectedStudent?._id}`, editStudent);
