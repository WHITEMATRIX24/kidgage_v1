...
const EditParentForm = () => {
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/personal/search', {
        params: { query }
      });
      if (response.data) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEditMode) {
      try {
        const response = await axios.put(`http://localhost:5000/api/personal/parent/${parentData._id}`, formData);
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(`http://localhost:5000/api/personal/parent/${parentData._id}`);
      setParentData(null);
      setFormData({
        username: '',
        email: '',
