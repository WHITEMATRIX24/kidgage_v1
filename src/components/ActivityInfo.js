...
const ActivityInfo = () => {
    ...
    useEffect(() => {
        ...
        const fetchCourseData = async () => {
            try {
                const courseResponse = await axios.get(`https://main.d15nt38h7hbizi.amplifyapp.com/api/courses/course/${courseId}`);
                setCourse(courseResponse.data);

                if (courseResponse.data && courseResponse.data.providerId) {
                    const providerResponse = await axios.get(`https://main.d15nt38h7hbizi.amplifyapp.com/api/users/provider/${courseResponse.data.providerId}`);
